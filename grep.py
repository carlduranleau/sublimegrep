...
class GrepCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		sublime.active_window().show_input_panel("Search string:", "", self.on_Search_String, None, None)
		pass
	def on_Search_String(self, user_input):
		try:
			if sublime.active_window():
				sublime.active_window().run_command("grep_engine", {"query": user_input} )
		except ValueError as e:
			sublime.status_message(e)

class GrepEngineCommand(sublime_plugin.TextCommand):
